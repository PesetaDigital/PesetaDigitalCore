...
void Shutdown(void* parg)
{
    ...
    RenameThread("PesetaDigital-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("PesetaDigital version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  PesetaDigitald [options]                     " + "\n" +
                  "  PesetaDigitald [options] <command> [params]  " + _("Send command to -server or PesetaDigitald") + "\n" +
                  "  PesetaDigitald [options] help                " + _("List commands") + "\n" +
                  "  PesetaDigitald [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "pesetadigital:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("PesetaDigital"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("PesetaDigital"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: PesetaDigital.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: PesetaDigitald.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 16639 or testnet: 26639)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 16638 or testnet: 26638)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  PesetaDigital is probably already running."), strDataDir.c_str()));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("PesetaDigital version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "PesetaDigital server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("PesetaDigital"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (mapArgs.count("-reservebalance")) // PesetaDigital: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // PesetaDigital: checkpoint master priv key
    {
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of PesetaDigital") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart PesetaDigital to complete") << "\n";
