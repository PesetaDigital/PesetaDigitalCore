...
// PesetaDigital: total coins staked (non-spendable until maturity)
...
bool CWallet::StakeForCharity()
{
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
    {
        ...
        if (pcoin->IsCoinStake() && pcoin->GetBlocksToMaturity() == 0  && pcoin->GetDepthInMainChain() == nCoinbaseMaturity+10)
        {
            ...
            {
                SendMoneyToDestination(strStakeForCharityAddress.Get(), nNet, wtx, false); // PTD removed 2 args  , "", true);
...
// Pesetadigital: get current stake weight
...
// PesetaDigital: check 'spent' consistency between wallet and txindex
// PesetaDigital: fix wallet spent state according to txindex
...
// PesetaDigital: disable transaction (only for coinstake)
