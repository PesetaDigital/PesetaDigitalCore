...
static const int64_t nTargetTimespan = 10 * 5 * 60;

static const unsigned int CHECKLOCKTIMEVERIFY_SWITCH_TIME = 1461110400; // Wednesday, 20-Apr-16 00:00:00 UTC

int64_t devCoin = 1 * COIN; // each pow block, 1 coin to official faucet
...
const string strMessageMagic = "PesetaDigital Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // PesetaDigital: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // PesetaDigital: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    int64_t nSubsidy = 25 * COIN; // pow reward  LAST_POW_BLOCK = 30000
    if(pindexBest->nHeight < 1)
    {
        nSubsidy = 143508000 * COIN; //PREMINED FOR SWAP PTC TO PTD. SAME QUANTITY PTC AND PTD 1 MAY 2021
    }


    // LAST_POW_BLOCK = 30000
...
const int YEARLY_BLOCKCOUNT = 36500;
...
// PesetaDigital: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes PesetaDigital fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // PesetaDigital: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // PesetaDigital: clean up wallet after disconnecting coinstake
    BOOST_FOREACH(CTransaction& tx, vtx)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // PesetaDigital: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // PesetaDigital: track money supply and mint amount info
...
// PesetaDigital: total coin age spent in transaction, in the unit of coin-days.
...
// PesetaDigital: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // PesetaDigital: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // PesetaDigital: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // PesetaDigital: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // PesetaDigital: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig, int height) const
{
    ...
    if(pindexBest != NULL && pindexBest->nHeight > 1)
        nCoinbaseMaturity = 100; //coinbase maturity change to 100 blocks
    // Size limits
    ...
    if (IsProofOfStake())
    {
        ...
        // Pesetadigital: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfWork() && nHeight > LAST_POW_BLOCK && !fTestNet)
    ...
    // PesetaDigital: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // PesetaDigital: check proof-of-stake
    ...
    // PesetaDigital: ask for pending sync-checkpoint if any
...
printf("ProcessBlock: ORPHAN BLOCK %lu, prev=%s\n", (unsigned long)mapOrphanBlocks.size(), pblock->hashPrevBlock.ToString().substr(0,20).c_str());
  if (pfrom) {
      ...
      // Ask this guy to fill in what we're missing
          ...
          // PesetaDigital: getblocks may not obtain the ancestor block rejected
    ...
    // PesetaDigital: if responsible for sync-checkpoint send it
...
// pesetadigital: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PesetaDigital", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "6/1/2021 20:52:49 siete años del primer bloque de Pesetacoin y bloque génesis de PesetaDigital";
        CTransaction txNew;
        txNew.nTime = 1609966369;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xde6ee1e64e40ab7391e5fee0cbf3eab40d1140d3423719b0a7abdc3c284b48a0"));
        ...
        // PesetaDigital: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // PesetaDigital: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // PesetaDigital: tell downloading node about the latest block if it's
